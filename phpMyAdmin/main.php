...
$sitename = preg_replace("/\.p[0-9]+\.hostingprod\.com/","",$_SERVER['SERVER_NAME']);
$parts = explode(".",$sitename);
$link = $parts[count($parts)-2].".".$parts[count($parts)-1];
?>
<div style="background-color:#ccccff;overflow:hidden;*height:80px;padding:20px;">
<h1 style="float:left;font-size:28px;width:70%;padding:0;margin:0"><?php echo $sitename;?></h1>
<div style="float:right;background-color:#ffffff;padding:10px;font-weight:bold"><a href="http://rd.webhosting.yahoo.com/bounce.php?d=<?=$link;?>" target=new>Control Panel</a></div>
</div>
<?php
